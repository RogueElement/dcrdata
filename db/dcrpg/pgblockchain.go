...
// TicketsInPool returns ...
func (pgb *ChainDB) TicketsInPool(limit, offset int64) ([]explorer.TicketPoolTx, error) {
	bb, err := pgb.HeightDB()
	if err != nil {
		return nil, err
	}

	_, hashes, heights, prices, fees, err := RetrieveTicketsInPool(pgb.db, bb-uint64(pgb.chainParams.TicketMaturity), limit, offset)
	if err != nil {
		return nil, err
	}

	tickets := make([]explorer.TicketPoolTx, 0, len(hashes))

	for i := range hashes {
		tickets = append(tickets, explorer.TicketPoolTx{
			Hash:        hashes[i],
